  ...
  test("captures one source-honest, responsive frame for every §18 object", async ({ page, context }) => {
    ...
    for (const frame of FRAMES) {
      ...
      if (frame.id === "12-agents-fleet") {
        expect(await page.getByRole("heading", { name: "Five real calling agents. Forty-four registered actions." }).count()).toBe(1)
        expect(await page.locator("[data-agent-fleet-rail] [data-agent-key]").count()).toBe(9)
        expect(await page.getByText("Assistant status unavailable").count()).toBeGreaterThan(0)
      }
