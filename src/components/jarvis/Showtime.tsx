    ...
    <main className="jarvis-root min-h-screen overflow-hidden bg-[#04070f] text-[color:var(--j-text)]" data-mood="idle" data-daypart="night">
      ...
      <section className="relative mx-auto flex min-h-screen max-w-7xl flex-col px-5 py-6 md:px-8">
        <header className="flex flex-wrap items-start justify-between gap-5 border-b border-white/10 pb-5">
          ...
          <Link href="/jarvis/bridge" className="rounded-full border border-white/15 px-3 py-1.5 j-fs-micro font-bold text-white/70 hover:border-cyan-200/60 hover:text-cyan-100">Return to Bridge</Link>
        </header>

        <div className="mt-5 flex flex-wrap items-center gap-3">
          <label className="j-fs-micro font-bold text-[color:var(--j-text-dim)]">Scenario <select value={scenario} onChange={(event) => setScenario(event.target.value as DealerZeroScenario)} className="ml-2 rounded-lg border border-white/12 bg-[#0b1423] px-2 py-1 text-white" disabled={playing}>{SCENARIOS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
          <button type="button" onClick={() => void loadDemo()} className="inline-flex items-center gap-2 rounded-full bg-cyan-300 px-3 py-1.5 j-fs-micro font-black text-slate-950 hover:bg-cyan-200"><Sparkles className="h-3.5 w-3.5" />{demo ? "Restart 60× demo" : "Start 60× demo"}</button>
          {demo && <button type="button" onClick={() => { if (!playing) startedAt.current = performance.now() - elapsed; setPlaying((value) => !value) }} className="inline-flex items-center gap-2 rounded-full border border-white/15 px-3 py-1.5 j-fs-micro font-bold text-white/80">{playing ? <><CirclePause className="h-3.5 w-3.5" />Pause</> : <><CirclePlay className="h-3.5 w-3.5" />Resume</>}</button>}
          {demo && !playing && elapsed >= demo.durationMs && <button type="button" onClick={() => { setElapsed(0); startedAt.current = performance.now(); setPlaying(true) }} className="inline-flex items-center gap-2 j-fs-micro font-bold text-cyan-200"><RotateCcw className="h-3.5 w-3.5" />Run again</button>}
        </div>
        ...
        {demo && <>
          <div className="mt-6 grid flex-1 gap-5 lg:grid-cols-[1.1fr_.9fr]">
            <section className="j-panel relative min-h-[420px] overflow-hidden p-5">
              <div className="flex items-start justify-between gap-4"><div><div className="j-label">NOW PLAYING</div><h2 className="mt-1 text-lg font-black text-white">{active?.label ?? "Loading script…"}</h2><p className="mt-1 j-fs-micro text-[color:var(--j-text-dim)]">{Math.round(elapsed / 100) / 10}s of {Math.round(demo.durationMs / 100) / 10}s · {demo.multiplier}× presentation speed</p></div><div className="h-24 w-24"><Orb3D live={{ state: frameState(active), activeRunCount: active?.kind === "workflow" ? 1 : 0 }} /></div></div>
              <div className="relative mt-12 h-40 overflow-hidden rounded-2xl border border-cyan-200/15 bg-[radial-gradient(circle_at_50%_115%,rgba(34,211,238,.28),transparent_45%),#071321]">
                ...
                <div className="absolute bottom-12 left-[14%] j-fs-micro font-black uppercase tracking-widest text-cyan-100/60">calls</div><div className="absolute bottom-12 left-[47%] j-fs-micro font-black uppercase tracking-widest text-cyan-100/60">gate</div><div className="absolute bottom-12 right-[12%] j-fs-micro font-black uppercase tracking-widest text-cyan-100/60">workflow</div>
              </div>
              <p className="mt-4 j-fs-micro leading-relaxed text-[color:var(--j-text-dim)]">The route line is visual timing for this scripted synthetic day, not a claimed road route or live operational movement.</p>
            </section>
            ...
            <section className="j-panel p-5"><div className="j-label">DAY TIMELINE</div><ol className="mt-4 space-y-3">{demo.frames.map((frame, index) => {
              ...
              return <li key={`${frame.atMs}-${frame.kind}`} className={`rounded-xl border p-3 transition ${reached ? "border-cyan-200/30 bg-cyan-300/[.06]" : "border-white/8 bg-white/[.015] opacity-55"}`}><div className="flex items-start gap-3"><span className={`mt-1 h-2 w-2 shrink-0 rounded-full ${reached ? "bg-cyan-200 shadow-[0_0_10px_rgba(103,232,249,.9)]" : "bg-white/25"}`} /><div className="min-w-0 flex-1"><div className="j-fs-micro font-bold text-white">{frame.label}</div><div className="mt-1 j-fs-micro text-[color:var(--j-text-dim)]">{(frame.atMs / 1000).toFixed(1)}s · {frame.kind.replaceAll("_", " ")}</div></div>{inspectable ? <button type="button" onClick={() => { setPlaying(false); setReceiptId(frame.receiptId) }} className="inline-flex shrink-0 items-center gap-1 rounded-full border border-cyan-200/30 px-2 py-1 j-fs-micro font-black text-cyan-100 hover:bg-cyan-200/10"><Eye className="h-3 w-3" />Inspect receipt</button> : <span className="shrink-0 j-fs-micro text-[color:var(--j-text-faint)]">No receipt linked</span>}</div></li>
            })}</ol><p className="mt-4 j-fs-micro leading-relaxed text-[color:var(--j-text-faint)]">“Inspect receipt” appears only when the tenant’s real receipt ledger supplied an ID. It is intentionally absent rather than fabricated.</p></section>
          </div>
