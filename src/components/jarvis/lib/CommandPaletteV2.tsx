...
function PaletteTab({ active, onClick, label }: { active: boolean; onClick: () => void; label: string }) { return <button onClick={onClick} className={`px-4 py-2 text-[10px] font-black uppercase tracking-wider ${active ? "border-b-2 border-cyan-300 text-cyan-200" : "text-white/40"}`}>{label}</button> }
