...
import { Command, FileText, Home, Search, Send, Workflow } from "lucide-react"
...
export function CommandPaletteV2({ onClose, onNavigate }: { onClose: () => void; onNavigate: (scene: "overview" | "pipeline") => void }) {
  const data = useJarvis(); const reduced = useReducedMotion(); const input = useRef<HTMLInputElement>(null)
  ...
  const navigation = useMemo(() => ([{ label: "Overview", scene: "overview" as const, icon: Home }, { label: "Pipeline theater", scene: "pipeline" as const, icon: Workflow }]).filter((item) => item.label.toLowerCase().includes(query.toLowerCase())), [query])
  ...
  return <AnimatePresence><motion.div className="fixed inset-0 z-[100] flex items-start justify-center bg-black/70 px-4 pt-[12vh]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={onClose}><motion.section role="dialog" aria-modal="true" aria-label="Command palette" className="w-full max-w-2xl overflow-hidden rounded-2xl border border-cyan-300/30 bg-[#07101d] shadow-[0_25px_100px_rgba(0,0,0,.6)]" initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -10, opacity: 0 }} transition={reduced ? { duration: .1 } : { type: "spring", stiffness: 340, damping: 28 }} onMouseDown={(event) => event.stopPropagation()}>
    ...
    <div className="flex items-center gap-2 border-b border-white/10 px-4 py-3"><Command className="h-4 w-4 text-cyan-300"/><input ref={input} value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => { if (e.key === "Escape") onClose(); if (e.key === "Enter") { e.preventDefault(); submit() } }} placeholder={mode === "instruct" ? "Describe what you need — JARVIS will propose actions for approval" : mode === "search" ? "Search receipts, households, and runs" : "Jump to a scene"} className="h-7 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-white/35"/><Press className="rounded-lg"><button onClick={submit} disabled={busy} className="rounded-lg bg-cyan-300 px-2.5 py-1 text-[10px] font-black text-slate-950 disabled:opacity-40">{mode === "instruct" ? <Send className="h-3.5 w-3.5"/> : <Search className="h-3.5 w-3.5"/>}</button></Press></div>
    <div className="max-h-[55vh] overflow-y-auto p-3">{error && <ErrorState message={error} onRetry={submit}/>} {mode === "navigate" && navigation.map(({ label, scene, icon: Icon }) => <button key={scene} onClick={() => { onNavigate(scene); onClose() }} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm text-white hover:bg-cyan-300/10"><Icon className="h-4 w-4 text-cyan-300"/>{label}</button>)} {mode === "search" && results.length > 0 && <div className="space-y-2">{results.map((result) => <div key={result} className="rounded-lg border border-white/8 bg-white/[.03] px-3 py-2 text-xs text-white/70">{result}</div>)}</div>} {mode === "instruct" && (planned.length ? <div className="space-y-3"><p className="text-[10px] text-teal-200">Proposed actions — press Enter on an action in the Approval Cockpit to review its gate. Nothing has executed here.</p>{planned.map((action) => <ActionRenderer key={action.id} actionType={action.actionType} payload={action.payload} compact />)}</div> : <EmptyState title="Describe the outcome" description="JARVIS will plan real actions, then send consequential work to the normal approval gate." />)}</div>
    <div className="border-t border-white/10 px-4 py-2 text-[9px] uppercase tracking-widest text-white/35">⌘K close · enter select · approval is always explicit</div>
  </motion.section></motion.div></AnimatePresence>
}
function PaletteTab({ active, onClick, label }: { active: boolean; onClick: () => void; label: string }) { return <Press><button onClick={onClick} className={`px-4 py-2 text-[10px] font-black uppercase tracking-wider ${active ? "border-b-2 border-cyan-300 text-cyan-200" : "text-white/40"}`}>{label}</button></Press> }
