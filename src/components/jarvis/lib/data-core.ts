...
export function JarvisDataProvider({ children }: { children: React.ReactNode }): React.ReactElement {
  ...
  useEffect(() => {
    ...
    // A background tab can mount already hidden. The ref used to default to true
    // until the first visibilitychange event, so every duplicate JARVIS tab fired
    // a full cold-start poll burst once. Seed it from the browser before starting
    // any lane; hidden lanes still schedule normally but perform no network work.
    onVisibility()
