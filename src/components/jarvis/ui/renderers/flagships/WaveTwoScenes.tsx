...
export function MaintenanceScene({ payload, compact }: ActionRendererProps) {
  ...
  return <Chrome label="Maintenance agreement" icon={HeartPulse} compact={compact}><div className="flex items-center gap-3"><motion.div className="grid h-12 w-12 shrink-0 place-items-center rounded-full border-2 border-teal-300/60 text-[11px] font-black text-teal-100" animate={reduced ? {} : { rotate: 360 }} transition={{ duration: 12, repeat: Infinity, ease: "linear" }}><span>AMC</span></motion.div><div className="min-w-0"><div className="truncate text-[12px] font-bold text-white">{text(p.householdLabel)}</div><div className="text-[10px] text-white/50">{cadence} renewal · confirmation required</div></div></div></Chrome>
}
...
export function ResearchScene({ actionType, payload, compact }: ActionRendererProps) {
  ...
  return <Chrome label="Web research" icon={Globe2} compact={compact}><div className="rounded-lg border border-violet-300/20 bg-violet-300/[.05] p-2"><div className="text-[11px] font-bold text-violet-100">{query}</div><div className="mt-1 text-[9px] text-white/45">Sources and extracted claims appear only after the real research action completes.</div></div></Chrome>
}
...
export function OpsOverviewScene({ payload, compact }: ActionRendererProps) {
  ...
  return <Chrome label="Operations overview" icon={HeartPulse} compact={compact}><div className="grid grid-cols-3 gap-1.5">{["Pipeline", "Cash", "Visits"].map((label) => <div key={label} className="rounded-lg border border-white/8 bg-white/[.025] p-2"><div className="text-[8px] uppercase tracking-wider text-white/40">{label}</div><div className="mt-1 text-[10px] font-bold text-cyan-100">live tile</div></div>)}</div><p className="mt-2 text-[10px] text-white/55">{question ? question : `Focus: ${text(p.focus, "current operations")}`}</p></Chrome>
}

export function ReminderScene({ payload, compact }: ActionRendererProps) {
  const p = payload as Record<string, unknown>; return <Chrome label="Service reminder" icon={Wrench} compact={compact}><div className="flex items-center gap-2 text-[10px]"><span className="h-2 w-2 rounded-full bg-teal-300"/><span className="text-white/75">Last serviced: {text(p.lastServicedAt)}</span><span className="h-px flex-1 bg-teal-300/30"/><span className="text-amber-200">due check</span></div><div className="mt-2 text-[11px] font-semibold text-white">{text(p.equipmentType)} </div></Chrome>
}
...
export function TechnicianReportScene({ actionType, payload, compact }: ActionRendererProps) {
  ...
  return <Chrome label="Technician report" icon={Wrench} compact={compact}><div className="border-l-2 border-cyan-300/70 pl-2.5 text-[11px] leading-relaxed text-white/75">{report}</div>{actionType === "log_visit_report" && <div className="mt-2 flex items-center gap-1.5 text-[9px] text-teal-200"><StatusDot status="ok" />{p.markCompleted === true ? "Completion requested" : "Report only"}</div>}</Chrome>
}
...
export function ComplianceScene({ payload, compact }: ActionRendererProps) {
  ...
  return <Chrome label="Compliance document" icon={ShieldCheck} compact={compact}><div className="rounded-lg border border-amber-300/20 bg-amber-300/[.04] p-2"><div className="flex items-center justify-between text-[10px] font-bold text-amber-100"><span>{text(p.householdLabel)}</span><span>preview 1 / 1</span></div><div className="mt-2 space-y-1">{profile.slice(0, 3).map(([key, value]) => <div key={key} className="flex justify-between text-[9px] text-white/55"><span>{key.replaceAll("_", " ")}</span><span>{String(value)}</span></div>)}</div></div><p className="mt-2 text-[9px] text-white/40">Generated document is available only after execution.</p></Chrome>
}
...
export function MarketingScene({ actionType, payload, compact }: ActionRendererProps) {
  ...
  return <Chrome label="Marketing" icon={Megaphone} compact={compact}><div className="flex items-center justify-between gap-2"><div className="min-w-0 truncate text-[11px] font-bold text-white">{label}</div><span className="shrink-0 rounded-full border border-violet-300/30 bg-violet-300/10 px-1.5 py-0.5 text-[8px] font-black text-violet-200">EMU</span></div><p className="mt-1.5 text-[9px] text-white/45">Emulator/sandbox status is explicit; no campaign or request has been sent by this proposal.</p></Chrome>
}
...
export function WaveTwoScene(props: ActionRendererProps) {
  switch (props.actionType) {
    ...
    default: return <Chrome label="Action scene" icon={FileText} compact={props.compact}><Enter><span className="text-[11px] text-white/60">{props.actionType}</span></Enter></Chrome>
  }
}
