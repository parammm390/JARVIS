  ...
  return (
    <FlowCard id="FLOW-67" title="WaveformTruth" reducedFallback="static level bars, no scroll">
      <div className="flex w-full flex-col items-center gap-2">
        ...
        <p className="text-[9px] text-white/30">FIXTURE oscillator here — the real one is LiveCallPanel&apos;s Vapi `local-volume-level`, never synthetic in production.</p>
      </div>
    </FlowCard>
  )
}
    ...
    <FlowCard id="FLOW-68" title="TranscriptTide" reducedFallback="line appears instantly, no enter animation">
      <div className="flex w-full flex-col items-center gap-2">
        <div className="w-full space-y-1.5">
          {lines.map((m, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="flex gap-2 text-[11px]">
        ...
        <p className="text-[9px] text-white/30">Real Vapi `message` events carry no per-word timestamps (grepped) — honest line-enter fallback, not fabricated word timing.</p>
      </div>
    </FlowCard>
  )
}
...
function IntentSparkDemo() {
  ...
  return (
    <FlowCard id="FLOW-69" title="IntentSpark" reducedFallback="chip appears in the tray directly, no flight">
      <div className="flex w-full flex-col items-center gap-2">
        <div ref={sourceRef} className="text-[10px] text-white/50">
        ...
        />
        <p className="text-[9px] text-white/30">FIXTURE action — real tray matches genuine pending domain_actions created since the call started (time-window, no stored call id).</p>
      </div>
    </FlowCard>
  )
}
  ...
  return (
    <FlowCard id="FLOW-70" title="CallOrbit" reducedFallback="static ring, no rotation">
      <div className="flex w-full flex-col items-center gap-2">
        ...
        <p className="text-[9px] text-white/30">DOM ring (SVG transform), not WebGL — LiveCallPanel&apos;s own orb container, not Bridge&apos;s Orb3D.</p>
      </div>
    </FlowCard>
  )
}
  ...
  return (
    <FlowCard id="FLOW-71" title="VoiceMoodWash" reducedFallback="instant swap, no 2s transition">
      <div className="flex w-full flex-col items-center gap-2">
        ...
        <p className="text-[9px] text-white/30">Real target is `[data-jarvis-atmosphere]`&apos;s own 2s opacity transition, driven by the real `data-mood` attribute (deriveMood()).</p>
      </div>
    </FlowCard>
  )
}
    ...
    <FlowCard id="FLOW-73" title="HangupSettle" reducedFallback="orbit body fades in place, no flight">
      <div className="flex w-full flex-col items-center gap-3">
        <div className="flex w-full items-center justify-between">
          ...
          <div ref={trayRef} className="h-8 w-16 rounded-lg border border-white/10 bg-white/5 text-center text-[8px] leading-8 text-white/40">
            feed
          </div>
        </div>
        ...
        <p className="text-[9px] text-white/30">Real flight targets ActivityRail&apos;s registered &quot;legacy-activity-rail&quot; anchor (pulse-bus registry, no new transport).</p>
      </div>
    </FlowCard>
  )
}
