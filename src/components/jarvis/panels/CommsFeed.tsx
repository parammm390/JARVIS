    ...
    <div className="j-panel">
      ...
      <div className="px-4 py-3">
        <div className="space-y-2">
          {items.length === 0 && <div className="j-fs-sm text-[color:var(--j-text-faint)]">No messages yet.</div>}
          {items.map((c) => {
            ...
            return (
              <motion.div key={c.id} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} className="rounded-lg border border-white/8 bg-white/[0.015] p-2.5">
                <div className="flex items-center gap-2 j-fs-micro font-bold uppercase text-[color:var(--j-text-dim)]">
                ...
                <div className="mt-1 truncate j-fs-micro text-[color:var(--j-text)]">{c.content.slice(0, 90)}</div>
                {c.simulated && <span className="mt-1 inline-block rounded-full bg-amber-300/12 px-2 py-0.5 j-fs-micro font-black text-amber-200">simulated delivery</span>}
              </motion.div>
            )
          })}
        </div>
