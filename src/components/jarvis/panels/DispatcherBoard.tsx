    ...
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
      <div className="j-panel">
        ...
        <div className="space-y-2 px-4 py-3">
          {!data.technicianLoad && <div className="jarvis-skeleton-tide h-16 rounded-lg bg-white/5" />}
          {data.technicianLoad?.length === 0 && <div className="j-fs-sm text-[color:var(--j-text-dim)]">No technicians on file.</div>}
          {data.technicianLoad?.map((t) => (
            <div key={t.technicianId} className="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.02] px-3 py-2">
              <span className="j-fs-sm font-bold text-[color:var(--j-text)]">{t.name}</span>
              <div className="flex gap-1.5">
                <span className="rounded-full bg-cyan-300/12 px-2 py-0.5 j-fs-micro font-black text-cyan-200">{t.upcomingAppointments} upcoming</span>
                <span className="rounded-full bg-white/8 px-2 py-0.5 j-fs-micro font-black text-white/50">{t.openWorkOrders} open orders</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2 px-4 py-3">
          ...
          {overdue.map((v) => (
            <div key={v.id} className="rounded-xl border border-red-400/25 bg-red-400/5 px-3 py-2">
              <div className="flex items-center justify-between j-fs-micro font-bold text-red-300">
        ...
        <div className="j-panel xl:col-span-2">
          ...
          <div className="flex flex-wrap gap-2 px-4 py-3">
            {data.serviceDue.slice(0, 12).map((s) => (
              <span key={s.agreementId} className="rounded-full bg-amber-300/12 px-2.5 py-1 j-fs-micro font-bold text-amber-200">
