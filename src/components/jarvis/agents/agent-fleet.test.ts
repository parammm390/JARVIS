...
describe("P3.T2 Agent Fleet contract", () => {
  ...
  it("keeps provider health separate from agent status", () => {
    ...
    expect(assistantStatusCopy(undefined, "loading")).toMatchObject({ label: "Assistant check in progress", tone: "loading" })
    expect(assistantStatusCopy(undefined, "idle")).toMatchObject({ label: "Assistant status not read", tone: "unavailable" })
    expect(providerStatusCopy(undefined, "loading")).toMatchObject({ label: "Vapi provider check in progress", tone: "loading" })
    expect(providerStatusCopy(undefined, "idle")).toMatchObject({ label: "Vapi provider status not read", tone: "unavailable" })
