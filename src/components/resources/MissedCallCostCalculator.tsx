  ...
  return (
    <ResourceFrame>
      <ResourceHero
        ...
        aside={
          <CalculatorSummary
            monthly={result.monthlyJobValueExposed}
            annual={result.annualJobValueExposed}
          />
        }
      />
          ...
          <div className="grid gap-6 lg:grid-cols-[0.92fr_1.08fr] lg:items-start">
            ...
            >
              ...
              <div className="mt-6 space-y-6">
                <SliderField
                  label="Weekly unworked leads / form leads"
                  value={weeklyCalls}
                  min={1}
                  max={150}
                  step={5}
                ...
                />
                <SliderField
                  label="Optional inbound percentage"
                  value={afterHours}
                  min={0}
                  max={100}
                  step={5}
            ...
            <div className="grid gap-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <ResultCard
                  icon={PhoneMissed}
                  label="Estimated unworked leads"
              ...
              >
                <div className="flex flex-col justify-between gap-6 md:flex-row md:items-start">
                  <div>
                    ...
                    <p className="mt-4 max-w-2xl text-sm font-semibold leading-relaxed text-slate-600 md:text-base">
                      With these assumptions, roughly{" "}
                      {number.format(result.missedCalls)} calls or form leads
                      receive no answer or slow follow-up each week. About{" "}
                      {number.format(result.afterHoursMissedCalls)}
                      of those may arrive after hours. The model applies your
                      selected opportunity rate and average job value only to
                      illustrate the size of the response gap.
                    </p>
                  </div>
                ...
                <div className="mt-7 grid gap-3 md:grid-cols-4">
                  {[
                    ...
                    <div
                      key={label}
                      className="rounded-2xl border border-slate-900/8 bg-white/72 p-4"
                    >
  ...
  return (
    <div className="ops-card relative overflow-hidden rounded-[2rem] p-6">
      ...
      <p className="mt-5 text-sm font-semibold leading-relaxed text-slate-600">
        Potential job value exposed by unanswered or slow-followed water leads
        under your selected assumptions.
      </p>
      <div className="mt-6 grid gap-3">
        <div className="rounded-2xl border border-teal-200 bg-teal-50 p-5">
          ...
          <AnimatedValue
            value={monthly}
            currencyValue
            className="mt-2 text-4xl font-black text-slate-950"
          />
        </div>
...
function ResultCard({
  ...
  return (
    ...
    >
      <div
        className={`mb-8 flex h-12 w-12 items-center justify-center rounded-2xl border ${toneClass}`}
      >
