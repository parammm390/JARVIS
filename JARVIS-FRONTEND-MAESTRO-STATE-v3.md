...
> 307,747 gzip bytes after route/state-boundary splits (down from 526,085 in a
...
receipt recovery contract that identifies the available authoritative operation
per failure (and its target), or an explicit plan amendment that makes the
receipt recovery presentation non-interactive for those kinds. P7.T1's
taxonomy mapping remains implemented, but the P7 exit-gate line “Every failure
kind offers a recovery affordance” and P7.T5 failure+recovery certification
remain open. **Who can unblock:** the plan owner/backend contract owner.
...
- [ ] **P7.T7** Perf: 5 cold Lighthouse desktop + mobile; bundle ≤ 250 KB gz; ≥ 55 fps; event→pixel median + p95
      **Evidence:** `59fe48b` · five cold production-build desktop runs: perf `60,95,95,94,95`, a11y `100×5`; five mobile runs: perf `72×5`, a11y `100×5`. P7 follow-up: `npm run build` then a Node gzip sum of `.next/app-build-manifest.json`'s `/jarvis/page` files measured **307,747 bytes** after deferring non-owner role scenes, the Supabase browser SDK, the Orb renderer, execution/receipt surfaces, and development-only fixture data from the initial graph—down from the fresh pre-change **526,085 bytes** (−218,338 bytes / 41.5%). The measured shared Framer Motion chunk alone is 38,251 gzip bytes. `npx tsc --noEmit && npm run lint` passed; the contradiction sweep → 14 passed and the previously recorded Thread fixture/public suites passed 14/14 and 5/5 under desktop Chromium. Full per-run metrics are in `docs/jarvis-v3-certification-2026-07-30.md`. **Deviation:** mobile perf still fails ≥85 and initial JS is still >250 KB; six-lane fps and event-to-pixel cannot be measured without B-5/B-6 evidence, so this task remains unchecked.
...
- **2026-07-30 · P7.T7 fresh production re-measurement, budget still failing.**
  ...
  **307,747 gzip bytes**, 41.5% below the fresh 526,085-byte baseline but still
  **57,747 bytes** above the fixed gate. The shared Framer Motion chunk is
  38,251 gzip bytes. No remaining source-backed role or fixture-only boundary
  can be split without changing required Thread behavior. **Next:** retain
  P7.T5/T6/T7 as unchecked until their actual contracts and measurements are
  satisfied; do not redefine the fixed gates.
...
- **2026-07-30 · P7.T5 recovery-surface audit, B-13 opened.**
  Verified the actual receipt call path: only integration recovery gets a
  functional Connect link. The other seven §6.8 affordances have no receipt
  operation or target in the current contract, so their buttons intentionally
...
- **2026-07-30 · P7.T7 production-route split, budget still failing.**
...
- **2026-07-30 · P7.T7 fixture-data split, budget still failing.**
  The test-only `?fixture=` data is now loaded only after the existing
  non-production fixture gate accepts a key, while retaining the visible
  `FIXTURE` label and the same real component tree. A fresh production build
  measured **303,715 gzip bytes**; the Thread fixture suite remained **14/14**.
  This is 53,715 bytes above the gate, so P7.T7 remains unchecked.
...
- **2026-07-30 · full Playwright verification run, not green.**
  ...
  comparisons: the two old signed-out `/jarvis` baselines, two old signed-out
  `/jarvis/next` baselines, and 22 legacy catalog-view snapshots. The failure
  artifacts identify no additional new functional test failure. No snapshot was
  updated or accepted, because the worktree already contains user-owned visual
  changes and changing a baseline is not a proof of correctness. The complete
  suite therefore remains red and cannot support certification.
