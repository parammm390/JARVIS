...
for (const invariant of [
  ...
  "docker run",
  "--network host",
  "FINNOR_WORKER_DEPLOYMENT_ID=ecs:container-smoke",
  "docker stop --time 30",
  "docker push",
...
if (!(credentialGateAt < oidcAt && oidcAt < dockerPushAt && dockerPushAt < preflightAt && preflightAt < stageApiAt && stageApiAt < stageFrontendAt && stageFrontendAt < captureAt && captureAt < migrationAt && migrationAt < workerDeployAt && workerDeployAt < browserInstallAt && browserInstallAt < stagedCertificationAt && stagedCertificationAt < promoteFrontendAt && promoteFrontendAt < promoteApiAt && promoteApiAt < parityAt && parityAt < smokeAt && smokeAt < rollbackAt)) fail("production workflow does not preserve build-once, pre-promotion certification, promotion, smoke, rollback order")
