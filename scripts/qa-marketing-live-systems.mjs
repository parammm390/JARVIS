...
await check("capability network selects recovery", "/capabilities", async () => { await page.locator("#capabilities-live").getByRole("button", { name: /Recover/i }).click(); await page.locator("#capabilities-live").getByText("safe route ready", { exact: true }).waitFor({ state: "visible" }); return true; }, Boolean);
await check("telemetry exposes provider failure", "/how-it-works", async () => { await page.locator("#how-live").getByRole("button", { name: /Provider timeout/i }).click(); await page.locator("#how-live").getByText("RECOVERY OPEN", { exact: true }).waitFor({ state: "visible" }); return true; }, Boolean);
await check("pricing scope toggles inventory", "/pricing", async () => { const button = page.locator("#pricing-live").getByRole("button", { name: /Inventory/i }); await button.click(); return (await button.getAttribute("aria-pressed")) === "false" && await page.locator("#pricing-live").getByText("Contact for pricing", { exact: true }).isVisible(); }, Boolean);
