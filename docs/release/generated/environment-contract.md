...
| Communications / SMS | required when external communications binding is selected | `COMMS_MODE`, `COMMUNICATIONS_BINDING`, `GMAIL_APP_PASSWORD`, `GMAIL_USER`, `RESEND_ALLOWLIST_OWNER_EMAIL`, `RESEND_API_KEY`, `RESEND_DAILY_CAP` | configured | `COMMS_MODE` |
| Resend / email | required when Resend is selected | `RESEND_ALLOWLIST_OWNER_EMAIL`, `RESEND_API_KEY`, `RESEND_DAILY_CAP` | configured | not applicable / not configured |
