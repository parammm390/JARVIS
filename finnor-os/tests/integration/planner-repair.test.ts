...
const HOUSEHOLD_ID = "22222222-2222-4222-8222-2222222222f9";
...
describe.skipIf(!available)("LLMPlanner repair pass", () => {
  beforeAll(async () => {
    ...
    await withTenant(TENANT_ID, async (db) => {
      await db.insert(households).values({ id: HOUSEHOLD_ID, tenantId: TENANT_ID, address: "11 Grounded Fixture Rd" }).onConflictDoNothing();
      await db.insert(proposals).values({ id: PROPOSAL_ID, tenantId: TENANT_ID, householdId: HOUSEHOLD_ID, status: "draft", content: {} }).onConflictDoNothing();
    });
  });
