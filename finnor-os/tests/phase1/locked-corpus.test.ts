...
const compiler = (grounded = true) => new IrAdmissibilityCompiler({
  ...
  evaluateConstraint: async (constraint) => ({
    truth: constraint.id.startsWith("constraint-") ? "violated" as const : "satisfied" as const,
    source: "canonical_state" as const,
    evidence: [`locked-fixture:${constraint.id}`],
    reason: "derived from the immutable mechanical fixture truth table",
    sourceVersions: { fixture: "phase1-1.0.0" },
  }),
