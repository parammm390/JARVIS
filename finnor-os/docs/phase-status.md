...
- Poison job replayed clean: new `tests/integration/poison-job-replay-drill.test.ts` (3/3) — a deliverer that genuinely fails 3 real times dead-letters for real; triage correctly recommends escalate (not a naive replay suggestion); an owner replays anyway via the real owner-gated route; a now-fixed deliverer actually delivers it clean via a real `relayOutboxEvents` call, not a simulated status flip.
