...
2. **A separate Railway (or Render) environment** for `apps/worker` and `apps/temporal-worker` — use `railway.staging.json` as the build/deploy config (identical to `railway.json`; kept as a separate file so staging and production deploy configs can diverge later without touching each other).
...
4. **A separate Temporal Cloud namespace** (or reuse one Temporal Cloud account with a `-staging` namespace) — set `TEMPORAL_NAMESPACE` accordingly; never run staging workflows against the production namespace.
...
- `TEMPORAL_NAMESPACE` → the staging namespace from step 4.
