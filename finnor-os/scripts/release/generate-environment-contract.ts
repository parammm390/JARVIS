...
const systems = [
  ...
  ["Railway API / worker / orchestrator", [/^RAILWAY_|^SERVICE_APP$/], "required for Railway deployment"],
  ...
  ["GLM provider family", [/^GLM_/], "required if selected by router"], ["Mistral", [/^MISTRAL_/], "required if selected by router"],
  ["DeepSeek", [/^DEEPSEEK_|^AWS_BEDROCK_DEEPSEEK_/], "required if selected by router"], ["Exa", [/^EXA_/], "required for web discovery"],
