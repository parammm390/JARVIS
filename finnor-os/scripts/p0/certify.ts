...
import { BASELINE_SHA, deterministicHash, P0_BRANCH, P0_RUNTIME_CORRECTION_PATHS, PREVIOUS_BASELINE_SHA, readJson } from "./lib";
...
async function validateRuntimeCorrections(manifest: RuntimeCorrectionManifest): Promise<void> {
  ...
  assert.equal(manifest.previousBaselineSha, PREVIOUS_BASELINE_SHA, "runtime correction ledger lost the previous P0 baseline");
  assert.equal(manifest.currentMainInherited, true, "runtime correction ledger must distinguish inherited current-main fixes from P0 changes");
  ...
  assert.equal(P0_RUNTIME_CORRECTION_PATHS.length, 0, "reconciled P0 unexpectedly changed a production runtime file");
}
...
async function validateContract(contract: SubstrateContract): Promise<void> {
  ...
  exactSet(contract.executionModels.map((model) => model.name), ["QUERY", "CONVERSATION", "ATOMIC_ACTION", "OBJECTIVE", "CLARIFY"], "execution model inventory drifted");
  ...
  for (const route of ["QUERY", "ATOMIC_ACTION", "OBJECTIVE", "CONVERSATION", "CLARIFY"]) assert.ok(routeSource.includes(`\"${route}\"`), `route ${route} missing from current type`);
