...
const P1_BASELINE_SHA = "8fcd8a1cebcf92791047777c0d9c70e95fc7aad2";
...
function validateReconciliationAndScope(): string[] {
  ...
  assert.equal(git(["merge-base", "HEAD", P1_FOUNDATION_SHA]), P1_FOUNDATION_SHA, "rebased pure P1 foundation is not an ancestor");
...
async function validateCorpora(): Promise<{ p0Cases: number; p1Cases: number; combinedCases: number; combinedHash: string; p1Hash: string; combinedUniqueSelectors: number }> {
  ...
  assert.equal(p1Selectors.length, 70);
  assert.equal(new Set(p1Selectors.map(selectorKey)).size, 69);
  ...
  assert.deepEqual(combinedDetails, { categoryCases: 55, selectorEntries: 110, uniqueSelectors: 109 });
  ...
  return { p0Cases: 24, p1Cases: 31, combinedCases: 55, combinedHash, p1Hash: p1.corpusHash, combinedUniqueSelectors: 109 };
}
