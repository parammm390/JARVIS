...
export async function bootstrapTenant(manifest: ClientManifest): Promise<BootstrapTenantResult> {
  ...
  try {
    ...
    // Serializes concurrent attempts for one client identity without blocking other
    // clients. The unique index remains the final database boundary.
    await client.query("SELECT pg_advisory_xact_lock(hashtextextended($1, 0))", [`client:${manifest.clientKey}`]);

    const existing = await client.query<{ id: string }>(
      "SELECT id FROM tenants WHERE client_key = $1",
      [manifest.clientKey],
    );
    if (existing.rows[0]) {
      ...
      await client.query(
        `UPDATE tenants
         SET name = $2, timezone = $3, owner_phone = $4
         WHERE id = $1
           AND (name, timezone, owner_phone) IS DISTINCT FROM ($2, $3, $4)`,
    ...
    for (const integration of manifest.integrations) {
      await client.query(
        ...
        [
          ...
          integration.credential?.provider ?? null,
          integration.credential?.ref.replaceAll("{tenantId}", tenantId) ?? null,
          integration.credential?.version ?? null,
          JSON.stringify(integration.credential?.metadata ?? {}),
        ],
      );
    }
    const capabilities = manifest.integrations.map((integration) => integration.capability);
    await client.query(
  ...
  // Policy convergence owns its own tenant transaction. If it fails, the stable
  // client identity/configuration above remains a safe partial checkpoint; rerunning
  // repairs the missing policies without minting a second tenant.
