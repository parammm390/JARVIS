...
const AREA_CODE = "319"; // Cedar Falls / Waterloo, IA — same real metro as the rest of this codebase's fixtures.
const DEALER_ZERO_SEED = 953187; // fixed — changing this reseeds a different (still deterministic) universe.
...
// ---------------------------------------------------------------------------
// Deterministic, per-entity hashed PRNG. rngFor(...parts) always returns the SAME
// generator for the SAME parts, regardless of call order or what else has run.
// ---------------------------------------------------------------------------
...
function rngFor(...parts: Array<string | number>): () => number {
...
function generateHousehold(keyPrefix: string, i: number): SyntheticHousehold {
  ...
  return {
    ...
    phone: `+1${AREA_CODE}555${phoneSuffix}`,
