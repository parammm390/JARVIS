...
interface Manifest { cases: Array<{ selectors: Array<{ file: string; title: string }> }>; }

const packageRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const osRoot = resolve(packageRoot, "../..");
const manifest = JSON.parse(readFileSync(join(packageRoot, "fixtures/p2-locked-cases.json"), "utf8")) as Manifest;
...
for (const [file, titles] of [...selectorsByFile.entries()].sort(([left], [right]) => left.localeCompare(right))) {
  if (!existsSync(join(osRoot, file))) throw new Error(`Locked P2 corpus file is missing: ${file}`);
  ...
  if (result.status !== 0) throw new Error(`Locked P2 corpus failed with exit code ${result.status}: ${file}`);
  const passed = Number([...(result.stdout ?? "").matchAll(/Tests\s+(\d+) passed/g)].at(-1)?.[1] ?? 0);
  if (passed !== titles.size) throw new Error(`Locked P2 corpus selected ${titles.size} tests in ${file}, but exactly ${passed} passed`);
  selectedTests += passed;
}

const integrityFile = "packages/operational-ir/src/p2-locked-corpus.test.ts";
...
if (integrity.status !== 0) throw new Error(`Locked P2 corpus integrity failed with exit code ${integrity.status}`);
const integrityPassed = Number([...(integrity.stdout ?? "").matchAll(/Tests\s+(\d+) passed/g)].at(-1)?.[1] ?? 0);
if (integrityPassed !== 1) throw new Error(`Locked P2 corpus expected one integrity proof, but ${integrityPassed} passed`);

console.log(`P2 locked extension PASS: ${manifest.cases.length} cases, ${selectedTests} unique deterministic selectors, 1 manifest/hash proof`);
