...
export interface InstructionRouteDecisionLike {
  version: 1;
  route: "QUERY" | "ATOMIC_EFFECT" | "OBJECTIVE" | "CONVERSATION";
  reasonCodes: string[];
  queryDecision?: unknown;
}

export interface InstructionRouteIrView {
  executionModel: Exclude<InstructionExecutionModel, "KNOWN_ACTION_COMPATIBILITY">;
  policyVersion: 1;
  reasonCodes: string[];
}
...
export type ExistingPlanningCandidate =
  ...
  | {
      kind: "domain_action";
      executionModel: "ATOMIC_EFFECT" | "KNOWN_ACTION_COMPATIBILITY";
      action: DomainAction;
...
export function adaptExistingPlanningCandidateToProgram(input: {
  ...
  if (input.candidate.kind === "instruction_route") {
    ...
    if (executionModel === "CONVERSATION") {
      return result("NOT_APPLICABLE", undefined, ["CONVERSATION has no operational computation and does not produce an OperationalProgram."], ["route classification"], ["no executable IR semantics exist"]);
    }
...
export const OPERATIONAL_IR_ADAPTER_MATRIX = [
  { representation: "HumanOperation", actualAtBaseline: false, toIr: "UNSUPPORTED", fromIr: "UNSUPPORTED", note: "No HumanOperation symbol exists in current production sources; InstructionRouteDecision and planner DomainAction[] are the audited seams." },
