...
describe("existing representation adapters", () => {
  it("maps the actual InstructionRouteDecision without inventing intent", () => {
    expect(adaptInstructionRouteDecision({ version: 1, route: "ATOMIC_EFFECT", reasonCodes: ["strict_single_effect_candidate"] })).toMatchObject({
      classification: "LOSSLESS",
      value: { executionModel: "ATOMIC_EFFECT", policyVersion: 1, reasonCodes: ["strict_single_effect_candidate"] },
    });
  ...
  it("records that HumanOperation does not exist at the audited baseline", () => {
    expect(OPERATIONAL_IR_ADAPTER_MATRIX.find((row) => row.representation === "HumanOperation")).toMatchObject({ actualAtBaseline: false, toIr: "UNSUPPORTED", fromIr: "UNSUPPORTED" });
  });
  ...
  it("assembles atomic DomainAction IR only when same-candidate desired-state semantics are explicit", () => {
    const missing = adaptExistingPlanningCandidateToProgram({
      candidate: { kind: "domain_action", executionModel: "ATOMIC_EFFECT", action: DOMAIN_ACTION },
    ...
    expect(adapted).toMatchObject({ classification: "LOSSY", value: { executionModel: "ATOMIC_EFFECT", irSemanticHash: atomicProgram().irSemanticHash } });
  });
  ...
  it("keeps conversation and missing route computations explicit instead of fabricating programs", () => {
    ...
    expect(adaptExistingPlanningCandidateToProgram({
      candidate: { kind: "instruction_route", decision: { version: 1, route: "ATOMIC_EFFECT", reasonCodes: ["strict_single_effect_candidate"] } },
      envelope,
...
describe("compatibility lowerer", () => {
  ...
  it("lowers the audited KNOWN_ACTION_COMPATIBILITY model without creating a new execution model", () => {
