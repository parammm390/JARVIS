...
export function initObservability(): void {
  ...
  Sentry.init({
    ...
    environment: process.env.FINNOR_ENVIRONMENT ?? process.env.NODE_ENV ?? "development",
    tracesSampleRate: process.env.SENTRY_DSN ? 0.1 : 0,
    // Production releases inject one provider-neutral, canonical commit identity.
    release: process.env.FINNOR_COMMIT_SHA,
  });
}
