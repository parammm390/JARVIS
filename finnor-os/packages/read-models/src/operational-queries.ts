...
/**
 * Execute one of the eight exact operational read intents. The request is never
...
export async function executeOperationalQuery<T extends OperationalQueryRequest>(
  ...
  try {
    const result = compatibilityEnvelope(await withTenant(tenantId, (db) => runOperationalQuery(db, tenantId, canonicalRequest, asOf, options)));
    if (claim) {
