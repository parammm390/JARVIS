...
const PACKAGE_ROOT = join(OS_ROOT, "packages/program-search");
...
describe("P4 pure downward import boundary", () => {
  ...
  it("imports no orchestration, DB, Authority, Work, provider, computer, API, or UI package", () => {
    const violations = files(SOURCE_ROOT, (path) => path.endsWith(".ts") && !path.endsWith(".test.ts")).flatMap((path) =>
  ...
  it("contains no ambient randomness, clock, network, timer, environment, or persistence primitive", () => {
    const forbidden = /\b(?:Date\.now|new Date|Math\.random|randomUUID|fetch|XMLHttpRequest|setTimeout|setInterval)\b|\bprocess\.env\b/g;
    const violations = files(SOURCE_ROOT, (path) => path.endsWith(".ts") && !path.endsWith(".test.ts")).flatMap((path) =>
      [...readFileSync(path, "utf8").matchAll(forbidden)].map((match) => `${relative(OS_ROOT, path)}:${match[0]}`));
    expect(violations).toEqual([]);
  });

  it("adds no package cycle and has no reverse import from its lower layers", () => {
    const graph = packageGraph();
    expect(graph.get("@finnor/program-search")).toEqual(ALLOWED_DEPENDENCIES);
    for (const dependency of ALLOWED_DEPENDENCIES) expect(graph.get(dependency) ?? []).not.toContain("@finnor/program-search");
