...
| R-16 | P1 | Provider-backed planner live evaluation is not green; the workflow route was pinned to Groq and the harness was paced for the provider's TPM limit, but the complete recheck still returned failures/errors | P0 | 🔴 open | `docs/release/evidence/P0/p0-t6-planner-live-eval.txt`; paced recheck `p0-t6-planner-live-eval-paced-recheck.txt` |
...
- **2026-08-06 · P0.T6 · planner live evaluation is not green** · Preview provider metadata contained non-empty Groq/Bedrock credentials. The initial provider-backed disposable-local run scored 11/41 passed, 29 failed, 1 errored; the workflow route was missing for Groq. The workflow now pins planning and repair to Groq, and the harness has a validated provider-paced setting. The complete paced Groq-routed recheck reached all 41 scenarios but scored 17/41 passed, 9 failed, 15 errored; sanitized classification found rate-limit/TPM signatures among the errors and no auth/timeout signatures. A complete green run still needs an owner-approved provider route/capacity and any required planner-quality repair. This remains distinct from the local staging-suite repeat pass. Evidence: `p0-t6-planner-live-eval.txt`, `p0-t6-planner-live-eval-paced-recheck.txt`. No production provider or customer effect was used.
